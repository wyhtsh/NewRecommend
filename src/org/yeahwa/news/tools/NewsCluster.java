...
public class NewsCluster {
	...
	private static Map<String, Integer> kMeansCluster(
		...
		int iterNum = 0;// 迭代次数
		while (true) {
			System.out.print("Stage-3 news clustering iteration No." + (iterNum++) + ":");
			// 3、计算每个点和每个聚类中心的距离
	 ...
	 * Stage-3
	 * 
	 * K-means聚类算法的实现类，将对象集聚成K类 算法结束条件:当每个点最近的聚类中心点就是它所属的聚类中心点时，算法结束
	 * 
	 */
	public static void execute() {
		// Map<id,Map<keyword,weight>>
		int K = 400;// number of clusters
		Map<String, Map<String, Double>> allTestSampleMap = DataGetter.getNewsSample();
		Map<String, Integer> KmeansClusterResult = new TreeMap<String, Integer>();
		KmeansClusterResult = kMeansCluster(allTestSampleMap, K);
		DataSetter.saveClusteredNews(KmeansClusterResult);
